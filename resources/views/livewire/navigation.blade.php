  ...
  <div class="max-w-7xl mx-auto px-2 sm:px-6 lg:px-8">
    <div class="relative flex items-center justify-between h-16">
      ...
      <div class="flex-1 flex items-center justify-center sm:items-stretch sm:justify-start">
          ...
          <img class="block lg:hidden h-8 w-auto" src="{{ asset('images/logo.png') }}" alt="Workflow">
          <img class="hidden lg:block h-8 w-auto" src="{{ asset('images/logo.png') }}" alt="Workflow">
          @auth
            <h1 class="hidden lg:block text-white font-bold text-2xl px-2">{{ auth()->user()->name }}</h1>
          @else
            <h1 class="hidden lg:block text-white font-bold text-2xl px-2">KehilApp</h1>
          @endauth
        </div>
        </a>
          ...
          <div class="ml-3 relative" x-data="{ open:false }">
            ...
            <div x-show="open" x-on:click.away="open = false" class="origin-top-right absolute right-0 mt-2 w-48 rounded-md shadow-lg py-1 bg-white ring-1 ring-black ring-opacity-5" role="menu" aria-orientation="vertical" aria-labelledby="user-menu">
              ...
              <form method="POST" action="{{ route('logout') }}">
                @csrf
                <a href="{{ route('logout') }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100" role="menuitem" onclick="event.preventDefault(); this.closest('form').submit();">Cerrar sesión</a>
            </form>
            </div>
          </div>
        @else
